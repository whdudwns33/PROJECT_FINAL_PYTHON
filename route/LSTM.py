...
df = pd.read_csv("stock_20230901_20240110.csv")

input_df = df[(df['종목코드'] == ticker) & (df['종가'] != 0)]
# 종가 기준 파싱
# reshape(-1, 1)는 열 데이터를 행 데이터로 변환
data = input_df['종가'].values.reshape(-1, 1)
print("data",len(data))

# 데이터의 총 수 - 1 : 현제 기(term)에서 몇 전기 까지 볼껀지
# 80%만 사용, 나머지는 검증에 사용 됨
# length = int(len(data)*0.8 - 1)
length = 60
# print("length",length)
...
print(future_predictions[0:5])



# Plot the results
# 학습용 80%
# train = input_df[:train_size]
# 검증용 20%
# valid = input_df[train_size:]
...
# plt.xticks(np.arange(0, len(valid['날짜']), step=5), rotation=45)
